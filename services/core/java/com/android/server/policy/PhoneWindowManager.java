...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private static final int MSG_TOGGLE_TORCH = 34;
    private static final int MSG_CLEAR_PROXIMITY = 35;
    ...
    public void beginLayoutLw(DisplayFrames displayFrames, int uiMode) {
        ...
        if (displayFrames.mDisplayId == DEFAULT_DISPLAY) {
            ...
            if (navVisible && mUseGestureButton && mGestureButton != null) {
    ...
    public void sendCustomAction(Intent intent) {
        String action = intent.getAction();
        if (action != null) {
            if (PixysUtils.INTENT_SCREENSHOT.equals(action)) {
                mContext.enforceCallingOrSelfPermission(Manifest.permission.ACCESS_SURFACE_FLINGER,
                        TAG + "sendCustomAction permission denied");
                mHandler.removeCallbacks(mScreenshotRunnable);
                mScreenshotRunnable.setScreenshotType(TAKE_SCREENSHOT_FULLSCREEN);
                mHandler.post(mScreenshotRunnable);
            } else if (PixysUtils.INTENT_REGION_SCREENSHOT.equals(action)) {
                mContext.enforceCallingOrSelfPermission(Manifest.permission.ACCESS_SURFACE_FLINGER,
                        TAG + "sendCustomAction permission denied");
                mHandler.removeCallbacks(mScreenshotRunnable);
